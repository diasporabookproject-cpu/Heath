...
- 🔴 **Approbation requise sur l'environnement `github-pages`** *(audit du 31/07, mesure 3 — action PO, 5 min)*. **Vérifié le 01/08 : l'environnement n'a AUCUNE restriction** (ni branches, ni relecteurs) — le chemin manuel était donc bien ouvert, contrairement à ce que j'avais inféré. Le **job `garde`** de `deploy.yml` refuse désormais toute réf autre que la branche désignée, mais **le code ne peut pas exiger un GO humain** : seul `Settings → Environments → github-pages → Required reviewers` le peut. Les mesures 1/4/5 sont posées ; celle-ci reste.
