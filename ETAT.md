...
**Lot simplification transverse** — branche `lot-simplification-v1`. **T1 livrée — ⏸ STOP T1** : purge nutrition (macros/calcium/objectif/flag `suivi` retirés du client ; `nutrition.ts`→`menu.ts` structurel ; seed + 30 recettes nettoyés) · fusion régime+allergies → `nePasManger` (halal reste toggle ; migration idempotente 2 portes ; **le double « sans » disparaît**) · **porte grep `kcal|macros?|calcium|prot|gluc`** en CI (critère de fini, zéro exemption) · Opt-C darija (schéma d'import allégé en T2). 165 tests · 3 smokes verts. **Reste : STOP dédié read-back prompt v2 → T2** (edge : `SYSTEM` hardcodé disparaît, seule fenêtre token). Fonds de départ éditorial + consignes personnel = HORS lot.
