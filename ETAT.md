# ÉTAT — Manzil
**Photo de l'état réel · réécrite à chaque clôture de tranche/lot · dernière touche : 20 juillet 2026 (clôture lot UI DA v2, merge `lot-ui-v1` → défaut)**
...
**Lot partage + suivi des tâches** — branche `lot-partage-v1` (`apk.yml` dessus). GO architecture PO acquis (journal insert-only `espace_checks` · LWW par item · patron `gouter?` · **policy select anon 0011 avec jointure d'existence sur `espaces`** — révocation vraie par construction, test bloquant dédié). **T1 CLOS** (feuille d'envoi v2 : « Partager avec », « Reçoit en الدارجة », bulle message, « Accès permanent » + QR frigo + copier le lien, registre neutre — toggle darija du message = brouillon assumé, hors T1). **T2 CODE-COMPLET** (0011 committée — policies par jeton vivant, exigence 🔴 dedans, protocole de preuve en fin de fichier ; lib LWW+file offline, 9 tests) — **fenêtre staging→prod EN ATTENTE du PO**. **T3 CLOS** (payload `cl?`/`tasks?` patron gouter? testé · feuille : switch + accordéon tâches, tag neutre « à cocher » · page : cases repas du jour + tâches, offline-first avec file et cache, aperçu inerte · non-traduction encodée — 219 tests). **Fenêtre 0011 CLOSE ✅** (staging+prod, 5 preuves sur la vraie base dont la 4 bloquante révoqué→select=0, parité 0, token mort 401). La remontée des coches (T3) est VIVE en prod. **T4 CODE-COMPLET** (aperçu = vue employeur lecture seule des vraies coches via `viewChecksToken` ; barre « N coché · vu HH:MM », non bloquant ; countDone testé — 222 tests). Retour peuplé à valider **device contre prod** (remontée vive). Reste : device PO → **clôture du lot**.

**Lot UI DA v2 « cœur testable » : CLOS le 20/07** — mergé au défaut (`lot-ui-v1`, merge --no-ff). Accueil B1 + Cuisine sur le socle DA v2 : terracotta 100 % sans, **emoji Fluent 3D embarqués** (43 repères, vocabulaire bento, zéro CDN — offline intact), B1 re-skinné (bande Aujourd'hui, héros 🥘, équipe avec **Retirer** direct, **tampon de build** visible), Cuisine complète — **4 moments** (goûter partout : page reçue « اللمجة », courses, digest ; `gouter?` optionnel à jamais, **lien perpétuel prouvé** device), **geste radial** (3 pétales, couche d'entrée, FAB inclus), **recette légère** (un nom suffit, prouvé bout-en-bout), Recettes vide/plein (sections par moment, favoris, collections en bas, **Supprimer** réel testé), plat **retirable**, « **Copier une journée** » au choix. **203 tests · 3 smokes CI**. Réf : `BRIEF_CUISINE_UX__CLAUDE_CODE.md` · récit : `DEVLOG.md`.
