...
**Lot UI DA v2 « cœur testable »** — branche `lot-ui-v1` (`apk.yml` pointé dessus). Accueil (B1) + Cuisine sur le socle DA v2 (terracotta, 100 % sans, emoji Fluent embarqué). **T0→T3 CLOS** : pack committé + `tokens.css` importé (T0) · **B1 re-skinné** (T1 : bande Aujourd'hui, héros + points, « Mon équipe » état-vide invitation, **Fluent EMBARQUÉ** 43 SVG preuve offline, purge `--mz-*`) · **Cuisine socle DA v2** (T2 : pastille Règles Q7 visible, sélecteur Semaine+jours défaut demain, partage bas, footer Fluent, audit pixel proto↔app) · **LES 4 MOMENTS** (T3 : `gouter?` **optionnel à jamais + gardes, zéro migration/fenêtre prod** — **2 tests bloquants verts** : lien perpétuel (page pré-goûter rend à l'identique) + sync ancien client (`DayMenu` sans clé ne plante rien) ; goûter partout — page reçue « اللمجة », courses, digest, accueil 16:30 🍪, créneau plat seul verrouillé par test ; cartes de moment tintées + vue semaine SPEC 4, libellés alignés Menu ↔ page reçue). **LE RADIAL + LA RECETTE LÉGÈRE** (T4 : 3 pétales — biblio à gauche Q5 — couche d'entrée qui ne contourne pas le composeur ; « ＋ Nouvelle recette » déplie les 3 voies EN PLACE ; porte UI levée — un NOM suffit, critère Q2 **prouvé** : yaourt sans ingrédient → posé → sur la page cuisinière, unitaire + smoke ; darija du nom seul parquée §7.4 sans perte). **RECETTES vide/plein** (T5 : chips mortes → sections par moment repliables Q3/Q4, tag de rôle mort, favoris toggle, collections EN BAS « Besoin d'inspiration ? », état vide = grandes tuiles + créer). 203 tests. **T0→T5 CODE-COMPLET + retours device PO n°1 et n°2 traités** (n°2 : emojis **Fluent 3D** — le set que la maquette bento rend sur Windows, +520 Ko assumés · Supprimer une recette avec purge/filtre testés · plat retirable + repas entrée-seule légal · « Copier UNE journée » au choix) (emojis alignés proto · Retirer depuis Mon équipe B1 · FAB→radial création · sélecteur = interface Recettes ; lien perpétuel validé device ✔). **Reste : re-test device → clôture du lot (merge au défaut).** Réf : `BRIEF_CUISINE_UX__CLAUDE_CODE.md` · le prototype cliquable **fait foi**.
