...
**🔒 LA PROD EST GARDÉE — plus aucune publication sans un accord humain (01/08).** Suite de l'incident du lot Identité et de l'audit systémique, les quatre chemins sont fermés : ① **approbation requise** sur l'environnement `github-pages` (*posée par le PO* — c'est elle qui exige le GO, aucune ligne de code ne peut le faire) · ② **portes partagées** `.github/workflows/portes.yml` appelées par `ci.yml` ET `deploy.yml` (typecheck · tests · build · **3 smokes**) : la prod n'est plus gardée moins sévèrement qu'une branche de travail, et il n'existe plus deux définitions qui dérivent · ③ job **`garde`** : une publication dont la réf n'est pas la branche que `deploy.yml` désigne est refusée (ferme `workflow_dispatch`) · ④ **`src/lib/portes.test.ts`** (5 cas, mutation-testé) : un lot **en vol** dont les workflows ne sont pas repointés fait rougir la CI. La règle d'ouverture de lot vit désormais dans **`CLAUDE.md` (règle n°2)**, seul fichier qu'une session neuve relit.
