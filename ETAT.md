...
**Lot UI DA v2 « cœur testable »** — branche `lot-ui-v1` (`apk.yml` pointé dessus). Accueil (B1) + Cuisine sur le socle DA v2 (terracotta, 100 % sans, emoji Fluent embarqué). **T0 + T1 + T2 CLOS** : pack committé + `tokens.css` importé (T0) · **B1 re-skinné** (bande Aujourd'hui, héros La Cuisine + points 4 jours toujours, duo, « Mon équipe » avec état-vide invitation, vouvoiement) · **emoji Fluent EMBARQUÉ** (43 SVG, preuve offline 0 CDN) · purge `--mz-*` (capture identique à l'octet) · **Cuisine socle DA v2 (T2)** : dégradé vert mort, pastille Règles (Q7 « Aucune restriction » visible) remplace ⚙, sélecteur Semaine+bande de jours (défaut demain, toggle proto), partage au bouton bas, footer Fluent, accent terracotta sur les actions (`--petrol` legacy meurt à T3-T5). Décisions gravées : `gouter?` **optionnel + gardes, pas de migration** (tolérance bidirectionnelle du modèle local — **zéro fenêtre prod**) · goûter = plat seul · « اللمجة » · 16:30 🍪 · recette légère = lever la porte UI `AddRecipeSheet` (modèle intact) · « Briefer » hors lot · « Gérer › » attend A7. Reste : **T2** Cuisine socle (en-tête + pastille règles, sélecteur, footer) → **T3 4 moments** (2 tests bloquants : lien perpétuel + sync ancien client) → **T4** radial + recette légère → **T5** Recettes. Réf : `BRIEF_CUISINE_UX__CLAUDE_CODE.md` · le prototype cliquable **fait foi**.
