...
**Lot partage + suivi des tâches** — branche `lot-partage-v1` (`apk.yml` dessus). GO architecture PO acquis (journal insert-only `espace_checks` · LWW par item · patron `gouter?` · **policy select anon 0011 avec jointure d'existence sur `espaces`** — révocation vraie par construction, test bloquant dédié). **T1 CLOS** (feuille d'envoi v2 : « Partager avec », « Reçoit en الدارجة », bulle message, « Accès permanent » + QR frigo + copier le lien, registre neutre — toggle darija du message = brouillon assumé, hors T1). **T2 CODE-COMPLET** (0011 committée — policies par jeton vivant, exigence 🔴 dedans, protocole de preuve en fin de fichier ; lib LWW+file offline, 9 tests) — **fenêtre staging→prod EN ATTENTE du PO**. **T3 CLOS** (payload `cl?`/`tasks?` patron gouter? testé · feuille : switch + accordéon tâches, tag neutre « à cocher » · page : cases repas du jour + tâches, offline-first avec file et cache, aperçu inerte · non-traduction encodée — 219 tests). **Fenêtre 0011 : STAGING fait + 5 preuves ✅** (idempotence ×2 · anon insert/select jeton vivant · insert jeton inconnu → RLS · **révoqué → select=0**, corollaire journal intact ; parité = delta `espace_checks` exact). **STOP — GO PROD du PO requis** → prod → révocation token → parité 0. Puis **T4** (retour employeur, sur table existante).
