...
**Lot simplification transverse** — branche `lot-simplification-v1`, `apk.yml` pointé. **Read-back livré (`READBACK_LOT_SIMPLIFICATION.md`) — ⏸ attente GO** (3 questions : séquencement · halal toggle ou fondu · `mealBudgets`). Cadre : app d'un foyer → app généraliste. **T1** purge nutrition (client — `SEED_CONFIG` et `nutrition.ts` sont MIXTES : structure reste, macros sortent · flag `suivi` = la carte, 68 refs · fusion régime+allergies en 1 champ `nePasManger` · porte grep `kcal|macro|calcium`) → **STOP dédié prompt v2** → **T2** prompt v2 (`SYSTEM` hardcodé disparaît, Opt-C darija VIABLE, seule fenêtre token). Fonds de départ éditorial + consignes personnel = HORS lot.
